...
import type { TArticleCategory, TError } from "../../../types";
...
const ArticleCategoryImageUploader = ({ item }: { item: TArticleCategory }) => {
    const [loading, setLoading] = useState(false);
    const [imageUrl, setImageUrl] = useState<string | undefined>(item?.image);
    type TUpdateArticleCategoryFromData = z.infer<
        typeof ArticleCategoryResolvers.updateArticleCategoryValidationSchema
    >;
    const { setValue } = useFormContext<TUpdateArticleCategoryFromData>();
...
const ArticleCategoriesUpdate = () => {
    const { articleCategoryID } = useParams();
    const navigate = useNavigate();
    const [category, setCategory] = useState<TArticleCategory | null>(null);

    const { data, isSuccess, isError, error } =
        articleCategoryService.useGetArticleCategoryQuery(articleCategoryID);
    ...
    const updateArticleCategoryHandler = async (
        data: TUpdateArticleCategoryFromData
    ) => {
    ...
    return (
        <div className="max-w-[520px] md:w-full md:mx-auto mb-10">
            ...
            {category ? (
                <Form<TUpdateArticleCategoryFromData>
                    onSubmit={updateArticleCategoryHandler}
                    defaultValues={defaultValues}
                    resolver={zodResolver(
                        ArticleCategoryResolvers.updateArticleCategoryValidationSchema
                    )}
                >
                    <ArticleCategoryImageUploader item={category} />
...
export default ArticleCategoriesUpdate;
