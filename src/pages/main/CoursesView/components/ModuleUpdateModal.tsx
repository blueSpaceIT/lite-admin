...
const ModuleUpdateModal = ({ module }: { module: TModule }) => {
    ...
    type TUpdateModuleFromData = z.infer<
        typeof ModuleResolvers.updateModuleValidationSchema
    >;

    const [updateModule] = moduleService.useUpdateModuleMutation();

    const updateModuleHandler = async (data: TUpdateModuleFromData) => {
        setIsOpen(false);
        const toastId = toast.loading("Wait a while");

        const result = await updateModule({ id: module.id, ...data });
        if (result?.error) {
                ...
                <div className="fixed inset-0 z-50 w-screen bg-black/40 overflow-y-auto">
                    <div className="flex min-h-full items-center justify-center p-4">
                        ...
                        >
                            ...
                            <Form<TUpdateModuleFromData>
                                onSubmit={updateModuleHandler}
                                defaultValues={defaultValues}
                                resolver={zodResolver(
                                    ModuleResolvers.updateModuleValidationSchema
                                )}
                            >
                                <InputField
                                    name="name"
